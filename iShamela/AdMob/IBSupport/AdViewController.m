...
- (NSString *)publisherIdForAd:(AdMobView *)adView {
	...
	return @"a14d4264d1e45ad"; // this should be prefilled; if not, get it from www.admob.com
#else
	return @"a14d42eb003e11d";
#endif
}
...
- (void)didReceiveAd:(AdMobView *)adView {
	...
	[UIView beginAnimations:@"admob" context:nil];
	[UIView setAnimationDuration:0.3];

	self.view.hidden = NO;

	DetailViewController *controller = (DetailViewController *)currentViewController;
	CGRect imageBound = controller.imageView.frame;
	imageBound.origin.y += adMobAd.bounds.size.height;
	imageBound.size.height -= adMobAd.frame.size.height;
	controller.imageView.frame = imageBound;

	CGRect webBound = controller.webView.frame;
	webBound.origin.y += adMobAd.bounds.size.height;
	webBound.size.height -= adMobAd.frame.size.height;
	controller.webView.frame = webBound;
