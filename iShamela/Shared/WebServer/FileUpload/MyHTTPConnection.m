...
- (NSString *)createBrowseableIndex:(NSString *)path
{
	...
	[outdata appendFormat:@"<title>%@ %@</title>", NSLocalizedString(@"iShamela", @"iShamela"), server.name];
	...
	[outdata appendFormat:@"<h1>%@ %@ %@</h1>", NSLocalizedString(@"iShamela", @"iShamela"), NSLocalizedString(@"Files", @"Files"), server.name];
    [outdata appendString:@"<p>"];
	[outdata appendFormat:@"<a href=\"..\">..</a><br />\n"];
    for (NSString *fname in array)
    {
        NSDictionary *fileDict = [[NSFileManager defaultManager] attributesOfItemAtPath:[path stringByAppendingPathComponent:fname] 
																		 error:&error];
	...
	if ([self supportsPOST:path withSize:0])
	{
		...
		[outdata appendFormat:@"<label>%@ ", NSLocalizedString(@"Upload File", @"Upload File")];
