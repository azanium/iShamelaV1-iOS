...
- (void)makeAdRequest:(BOOL)isFirstRequest {
  ...
  [currAdapter getAd];
	/*
  if ([currAdapter.adNetworkView isKindOfClass:AdMobView.class])
  {
	  AdMobView *admob = (AdMobView *)currAdapter.adNetworkView;
	  if ([admob.delegate respondsToSelector:@selector(currentViewController)])
	  {
		  admob.delegate.currentViewController = self.currentViewController;
		  NSLog(@"gotcha");
	  }
  }*/
}
